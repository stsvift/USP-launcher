...
const openForum = () => {
    ipcRenderer.send("openforum"
    );
}

const openSpace = () => {
    ipcRenderer.send("openspace"
    );
}

const openTS = () => {
    ipcRenderer.send("opents"
    );
}
...
function serverSwitcher(pos, lArrow, rArrow){
    ...
    totalPlayers.innerText = `Загрузка...`;
 ...
 regedit.list("HKCU\\SOFTWARE\\CitizenFX\\FiveM\\", function(err, result) {
    $.each(result, function(index, data) {
        try {
            ...
            $("#sv-join").prop("disabled", true).text("error").attr("title", "Кажется, у вас не установлен FiveM.");
        }
...
if(isOnline){
    ...
    ipcRenderer.on("listPlayers", (event, data) => {
        let totalPlayers = document.getElementById("totalPlayers");
        totalPlayers.innerText = `${data.length} Игрок(ов) подключено`;
        let plist = document.getElementById("plist");
        plist.textContent = "";
        if(data.length > 0){
            ...
            let totalPlayers = document.getElementById("totalPlayers");
            totalPlayers.innerText = `Сервер выключен!`;
            let plist = document.getElementById("plist");
            plist.textContent = "";
        }
    });
