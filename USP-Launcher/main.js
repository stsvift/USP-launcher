...
ipcMain.on('openforum', (event, data) => {
    const ps = new Shell();
    ps.addCommand("start " + JSON.stringify(CFG.forum));
    ps.invoke();
});

ipcMain.on('openspace', (event, data) => {
    const ps = new Shell();
    ps.addCommand("start " + JSON.stringify(CFG.space));
    ps.invoke();
});

ipcMain.on('opents', (event, data) => {
    const ps = new Shell();
    ps.addCommand("start " + JSON.stringify(CFG.ts));
    ps.invoke();
});
